    ...
    <div class="row mb-4">
        ...
        <div class="col-md-4 text-md-end">
            <div class="btn-group" role="group">
                <a href="{{ route('equipos.disponibles') }}" class="btn btn-primary">
            ...
            <form method="GET" class="row g-3 align-items-center">
                ...
                <div class="col-md-4">
                    <div class="input-group">
                        ...
                        <input type="text"
                               ...
                               placeholder="Buscar por empleado, computadora, código..."
                               value="{{ request('q') }}">
                    </div>
                </div>
                ...
                <div class="col-md-2">
                    <select name="equipo_id" class="form-select">
                        ...
                        @foreach($equiposFiltro ?? [] as $eq)
                            <option value="{{ $eq->id }}" {{ request('equipo_id') == $eq->id ? 'selected' : '' }}>
                                {{ $eq->codigo_interno }} - {{ $eq->marca }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-1">
                    <div class="d-flex gap-2">
                        ...
                        @if(request()->anyFilled(['q', 'estado', 'empleado_id', 'equipo_id']))
                            <a href="{{ route('asignaciones.dashboard') }}" class="btn btn-outline-secondary">
            ...
            @if(request()->anyFilled(['q', 'estado', 'empleado_id', 'equipo_id']))
                <div class="mt-3 d-flex align-items-center flex-wrap gap-2">
                    ...
                    @if(request('equipo_id'))
                        <span class="badge bg-info text-dark me-2">
                            <i class="bi bi-pc-display me-1"></i> Computadora ID: {{ request('equipo_id') }}
                        </span>
                    @endif
                </div>
            @endif
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($asignacion->equipo)
                                        <div class="fw-bold">{{ $asignacion->equipo->marca }} {{ $asignacion->equipo->modelo }}</div>
                                        <small class="text-muted d-block">
                                            <i class="bi bi-upc-scan me-1"></i>
                                            Código: {{ $asignacion->equipo->codigo_interno }}
                                        </small>
                                    @else
                                        <span class="text-danger">Computadora eliminada</span>
                                    @endif
                                </td>
                                ...
                                <td class="text-center">
                                    <div class="d-flex justify-content-center gap-2 flex-wrap">
                                        ...
                                        @if($asignacion->carta_pdf)
                                            ...
                                            <a href="{{ route('asignaciones.descargar', $asignacion->id) }}"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelector('select[name="equipo_id"]')?.addEventListener('change', function() {
            this.form.submit();
        });
        ...
        document.querySelectorAll('.btn-devolver').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                Swal.fire({
                    title: '¿Devolver computadora?',
                    text: 'Esta acción registrará la devolución del equipo',
                    icon: 'warning',
