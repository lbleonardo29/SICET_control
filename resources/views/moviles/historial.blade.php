...
@section('page-title', 'Historial del Dispositivo')
        ...
        <div class="col-md-8">
            <div class="d-flex align-items-center gap-3">
                ...
                <span class="badge bg-primary px-3 py-2">
                    ...
                    {{ $movil->codigo_interno }}
                </span>
            </div>
            <p class="text-muted mt-2">
                <i class="bi bi-info-circle me-1"></i>
                {{ $movil->marca }} {{ $movil->modelo }} - IMEI: {{ $movil->imei ?? 'N/A' }}
            </p>
        </div>
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h3 class="mb-0">{{ $historial->count() }}</h3>
                        </div>
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h3 class="mb-0">{{ $historial->whereNull('fecha_devolucion')->where('estado_asignacion', 'aceptada')->count() }}</h3>
                        </div>
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h3 class="mb-0">{{ $historial->whereNotNull('fecha_devolucion')->where('estado_asignacion', '!=', 'rechazada')->count() }}</h3>
                        </div>
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h3 class="mb-0">{{ $historial->where('estado_asignacion', 'rechazada')->count() }}</h3>
                        </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0" id="historialTable">
                    ...
                    <tbody>
                        @forelse($historial as $index => $a)
                        <tr>
                            <td class="px-3 fw-bold">{{ $loop->iteration }}</td>
                            
                            {{-- Empleado --}}
                            <td>
                                @if($a->empleado)
                                    <div class="fw-bold">{{ $a->empleado->nombre_completo ?? $a->usuario->name ?? 'Empleado' }}</div>
                                    @if($a->empleado->numero_empleado)
                            ...
                            <td class="text-center">
                                @if($a->carta_pdf && $a->estado_asignacion == 'aceptada')
                                    <a href="{{ route('asignaciones.moviles.descargar', $a->id) }}"
                        ...
                        <tr>
                            <td colspan="7" class="text-center py-5">
                                ...
                                <h4 class="text-muted">No hay historial para este dispositivo</h4>
                                <p class="text-muted mb-4">Este dispositivo no ha sido asignado a ningún empleado</p>
                                @if(auth()->user()->role === 'admin' && $movil->estado === 'Disponible')
                                    <a href="{{ route('asignaciones.moviles.create', $movil->id) }}" class="btn btn-primary">
...
<script>
    ...
    function exportToExcel() {
        ...
        a.href = url;
        a.download = 'historial_dispositivo_{{ $movil->codigo_interno }}.csv';
        a.click();
